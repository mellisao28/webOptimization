...
function changePizzaSizes(size){var pizzaContainer=document.getElementsByClassName("randomPizzaContainer");var dx=determineDx(pizzaContainer[0],size);var newwidth=(pizzaContainer[0].offsetWidth+dx)+'px';for(var i=0;i<pizzaContainer.length;i++){pizzaContainer[i].style.width=newwidth;}}
...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");var items=document.getElementsByClassName('mover');var cachedScroll=document.body.scrollTop / 1250;var phase=[];for(var i=0;i<5;i++){phase.push(Math.sin(cachedScroll+i)*100);}
for(var i=0,max=items.length;i<max;i++){items[i].style.left=items[i].basicLeft+phase[i%5]+'px';}}
...
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=8;var s=256;var pizzaHeight=100,pizzaWidth=73.33;var numOfMovingPizzas=(self.innerHeight*self.innerWidth)/(pizzaHeight*pizzaWidth);for(var i=0;i<numOfMovingPizzas;i++){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height=pizzaHeight+"px";elem.style.width=pizzaWidth+"px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i / cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem);}
updatePositions();});